...
import { Check, Zap, AlertCircle, UserCheck, ArrowRight } from 'lucide-react';
...
export default function PricingPage() {
  ...
  return (
    <div className="pt-20 sm:pt-32 pb-12 sm:pb-24 bg-[#FBFBFC] min-h-screen relative overflow-hidden">
      <SparkleContainer />
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-10 sm:mb-16">
          ...
          <h1 className="text-2xl sm:text-5xl font-extrabold text-[#0A0A0C] tracking-tight mt-3">
            Predictable Plans for Creators
          </h1>
          <p className="mt-3 text-[#71717A] text-xs sm:text-base font-normal leading-relaxed">
            Turn every single audio recording or video script into 7 days of ready-to-post social media content.
          </p>

          {user?.loggedIn && (
            <div className="mt-6 inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white border border-[#E4E4E7] shadow-2xs text-xs font-medium text-[#0A0A0C]">
              <UserCheck className="w-4 h-4 text-[#FF529A]" />
              <span>Signed in as <strong>{user.email}</strong></span>
              <span className="px-2.5 py-0.5 rounded-full bg-[#FF529A] text-white text-[10px] uppercase font-bold ml-1">
                {userTier} Active
              </span>
            </div>
          )}
        ...
        {/* Minimal Luxury Grid */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch mb-20 max-w-5xl mx-auto">
          {/* Card 1: Starter / Free */}
          <motion.div
            whileHover={{ y: -4 }}
