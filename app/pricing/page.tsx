...
import { Check, Zap, Crown, ShieldCheck, AlertCircle, UserCheck, Sparkles, Star, ArrowRight } from 'lucide-react';
...
export default function PricingPage() {
  ...
  return (
    <div className="pt-20 sm:pt-32 pb-12 sm:pb-24 bg-[#F5F5F7] min-h-screen relative overflow-hidden bg-aiigen-dots">
      ...
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[500px] bg-gradient-to-tr from-pink-300/30 via-purple-300/20 to-transparent blur-3xl pointer-events-none rounded-full" />

      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-10 sm:mb-16">
          <div className="aiigen-pill mb-2 sm:mb-3 border-[#FFC2DA] bg-white/80 backdrop-blur-md shadow-xs">
            <Sparkles className="w-3.5 h-3.5 text-[#FF529A] animate-pulse" />
            <span className="text-[#0A0A0C] text-[10px] sm:text-xs font-semibold">Transparent Creator Investment</span>
          </div>
          <h1 className="text-2xl sm:text-6xl font-extrabold text-[#0A0A0C] tracking-tight">
            Invest in Leverage. <span className="text-gradient-aura">Scale Your Audience.</span>
          </h1>
          <p className="mt-2 sm:mt-4 text-[#52525B] text-xs sm:text-xl font-medium leading-relaxed">
          ...
          {user?.loggedIn && (
            <div className="mt-6 inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white border border-[#FFC2DA] shadow-md text-xs font-bold text-[#0A0A0C]">
              <UserCheck className="w-4 h-4 text-[#FF529A]" />
              <span>Signed in as <strong>{user.email}</strong></span>
              <span className="px-2.5 py-0.5 rounded-full bg-[#FF529A] text-white text-[10px] uppercase font-extrabold ml-1 shadow-2xs">
                {userTier} Active
              </span>
            </div>
          )}
